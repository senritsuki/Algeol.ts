...
export function duplicateGeoWithAffine(g: Geo, m4: mx.M4[]): Geo[] {
	const verts = g.verts();
	const faces = g.faces();
	return m4.map(m => geo(verts.map(v => m.map_v3(v, 1)), faces))
}
/** 3次元→3次元のラムダ関数によるジオメトリ複製 */
export function duplicateGeoWithLambda(g: Geo, map: (v: vc.V3) => vc.V3[]): Geo[] {
	const faces = g.faces();
	return ut.transpose<vc.V3>(g.verts().map(map)).map(verts => geo(verts, faces));
}

/** データ配列を併用したアフィン写像の合成 */
export function compositeAffineWithDataArray<T>(data: T[], lambdas: Array<(d: T) => mx.M4>): mx.M4[] {
