...
function writeText(name: string, start: ls.Rule[], rec: number): void {
    const strs: string[] = [];
    for (let i = 0; i <= rec; i++) {
        const values = ls.l_system(start, i);
        const s = values.map(v => v.variable()).join('');
        strs.push(s);
    }
...
function writeSvg(name: string, start: ls.Rule[], rec: number, turtle: (vl: ls.Rule[], rec: number) => cv.Curve2[]): void {
    for (let i = 0; i <= rec; i++) {
        const values = ls.l_system(start, i);
