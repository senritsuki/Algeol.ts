...
function process_mid(keymap: Map<string, VI>, i1: number, i2: number, verts: vc.V3[], subverts: vc.V3[]): VI {
    const key = to_key(i1, i2);
    let vi = keymap.get(key);
    if (!vi) {
        const i = verts.length + subverts.length;
        const v = vc.add(verts[i1], verts[i2]).scalar(0.5);
        vi = {v, i}
        keymap.set(key, vi);
        subverts.push(v);
    }
...
export function verts_faces(r: number, recurse: number): VF {
    ...
    return verts_faces_recurse({verts, faces}, recurse);
}

function verts_faces_recurse(vf: VF, recurse: number): VF {
    if (recurse <= 0) {
        return vf;
    }
    ...
    faces.forEach(f => {
        ...
        const mvi = loop.map(ii => process_mid(keymap, f[ii[0]], f[ii[1]], verts, subverts));
    ...
    return verts_faces_recurse(sub_vf, recurse - 1);
}
