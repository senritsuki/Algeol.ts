...
/** 任意の平面形状の角柱 */
export function Prism(p2: prim.Primitive2): prim.Primitive3s {
    return prim.primitive3(v => {
        if (v.z < prism_z_min || v.z > prism_z_max) return false;
        return p2.bool(vc.v3_to_v2(v));
    }, 
    () => sfl.extrude2(p2.verts, prism_z_max));
}
/** 任意の平面形状の角錐 */
export function Cone(p2: prim.Primitive2): prim.Primitive3s {
    return prim.primitive3(v => {
        if (v.z < prism_z_min || v.z > prism_z_max) return false;
        if (v.z == prism_z_max) {
    ...
    () => sfl.extrude2_cone(p2.verts, prism_z_max));
}
