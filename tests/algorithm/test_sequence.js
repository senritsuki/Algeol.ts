...
function test() {
    return t.printModule('algeol/math/sequence', [
        { group: 'sequence', results: t.tests([
                ...
                t.dataNumArray('seq.bernstein(2, 0/4)', seq.bernstein(2, 0 / 4), [1, 0, 0]),
                t.dataNumArray('seq.bernstein(2, 1/4)', seq.bernstein(2, 1 / 4), [9 / 16, 6 / 16, 1 / 16]),
                t.dataNumArray('seq.bernstein(2, 2/4)', seq.bernstein(2, 2 / 4), [1 / 4, 2 / 4, 1 / 4]),
                t.dataNumArray('seq.bernstein(2, 4/4)', seq.bernstein(2, 4 / 4), [0, 0, 1]),
                t.dataNumArray('seq.bernstein(3, 0/4)', seq.bernstein(3, 0 / 4), [1, 0, 0, 0]),
                t.dataNumArray('seq.bernstein(3, 1/4)', seq.bernstein(3, 1 / 4), [27 / 64, 27 / 64, 9 / 64, 1 / 64]),
                t.dataNumArray('seq.bernstein(3, 2/4)', seq.bernstein(3, 2 / 4), [1 / 8, 3 / 8, 3 / 8, 1 / 8]),
                t.dataNumArray('seq.bernstein(3, 4/4)', seq.bernstein(3, 4 / 4), [0, 0, 0, 1]),
            ], t.evalNumArray) },
    ]);
}
exports.test = test;
