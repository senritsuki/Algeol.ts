...
exports.lvICube = function (v) { return mx.m3_rotate_y(-(ut.deg90 - Math.atan2(1, ut.r2))).mul(mx.m3_rotate_z(-ut.deg45)).map(v); };
exports.g1oHalfCube = prim.cube(0.5).map3(function (v) { return mx.m3_scale([1, 1, 0.5]).map(v); });
...
exports.g0cube = exports.g1zpICube.map3(function (v) { return mx.affine3_scale([0.5, 0.5, 0.5]).map_v3(v, 1); });
exports.g0crys4 = prim.tetrahedron(0.5).map3(function (v) { return mx.affine3_scale([0.5, 0.5, 1]).mul(mx.affine3_translate([0, 0, 0.5])).map_v3(v, 1); });
...
(function (node) {
    ...
    node.vxyFloor4 = al.duplicate_v3([
        ...
        vc.v3(0.5, 1.5, 0),
    ], 1, al.compose_v4map(seq.arith(4), [function (d) { return mx.affine3_rotate_z(ut.deg90 * d); }])).reduce(function (a, b) { return a.concat(b); }, []);
    node.vxyFloor6 = al.duplicate_v3([
        vc.v3(1.5, -0.5, 0),
        vc.v3(1.5, 0.5, 0),
    ], 1, al.compose_v4map(seq.arith(6), [function (d) { return mx.affine3_rotate_z(ut.deg60 * d); }])).reduce(function (a, b) { return a.concat(b); }, []);
    node.g4Floor = extrudeZZ(node.vxyFloor4, -nFloorD, 0);
    node.g6Floor = extrudeZZ(node.vxyFloor6, -nFloorD, 0);
    var rot_z_atan2_m4 = function (v) { return mx.affine3_rotate_z(Math.atan2(v.y, v.x)); };
    var vRing = prim.fn.circle.verts_i(6, nRingR);
    var gRing = extrudeZZ(vRing, -nRingD, nRingD);
    var mapRingFloor = function (v) { return mx.affine3_translate([0, 0, -nRingD]).mul(mx.affine3_scale([1, 1, 1.5])).map_v3(v, 1); };
    var mapRingArch = function (v) { return mx.affine3_translate([0, 0, nColumnH - nArchWallHeight]).mul(mx.affine3_scale([1, 1, 0.25])).map_v3(v, 1); };
    node.g4Rings = al.duplicate_f(gRing, al.compose_v4map(seq.arith(node.vxyFloor4.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, 0]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor4[i]); }]));
    node.g6Rings = al.duplicate_f(gRing, al.compose_v4map(seq.arith(node.vxyFloor6.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, 0]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor6[i]); }]));
    ...
    node.g4Columns = al.duplicate_f(gColumn, al.compose_v4map(seq.arith(node.vxyFloor4.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, 0]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor4[i]); }]));
    node.g6Columns = al.duplicate_f(gColumn, al.compose_v4map(seq.arith(node.vxyFloor6.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, 0]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor6[i]); }]));
    var gColumnUnder = antiprism(4, rpzl(4, 1 / 16, 1 / 32, seq.arith(7, -nUnderFloorD, (nUnderFloorD - 0.25) / 6)));
    node.g4ColumnUnders = al.duplicate_f(gColumnUnder, al.compose_v4map(seq.arith(node.vxyFloor4.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, 0]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor4[i]); }]));
    node.g6ColumnUnders = al.duplicate_f(gColumnUnder, al.compose_v4map(seq.arith(node.vxyFloor6.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, 0]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor6[i]); }]));
    ...
    var mapsRoofA = al.compose_m4([vc.v2(1, nColumnH), vc.v2(0.625, nColumnH + 0.75), vc.v2(1 / 64, nColumnH + nRoofH * 0.875)], [function (d) { return mx.affine3_scale([d.x, d.x, 1]); }, function (d) { return mx.affine3_translate([0, 0, d.y]); }]);
    ...
    node.g4RoofAs = al.duplicate_f(gRoofAs, al.compose_v4map(seq.arith(node.vxyFloor4.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, nColumnH]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor4[i]); }]));
    node.g6RoofAs = al.duplicate_f(gRoofAs, al.compose_v4map(seq.arith(node.vxyFloor6.length), [function (_) { return mx.affine3_translate([nr25 * nInner, 0, nColumnH]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor6[i]); }]));
    var mapsRoofB = al.compose_m4(seq.arith(12, 0, ut.deg90 / 12).map(function (rad) { return vc.v2(Math.cos(rad), nColumnH + nr25 * Math.sin(rad)); }), [function (d) { return mx.affine3_scale([d.x, d.x, 1]); }, function (d) { return mx.affine3_translate([0, 0, d.y]); }]);
    ...
    var gCrys = prim.bipyramid(4, nCrysR, ut.phi / (1 + ut.phi), 1 / (1 + ut.phi))
        .map3(function (v) { return mx.affine3_translate([0, 0, 1 / (1 + ut.phi)]).map_v3(v, 1); });
    var gObj1 = exports.g1oICube
        .map3(function (v) { return mx.affine3_translate([0, 0, nObjR / 2]).mul(mx.affine3_scale([nObjR, nObjR, nObjR])).map_v3(v, 1); });
    ...
    node.g4RoofC1 = al.duplicate_f(gCrys, al.compose_v4map(seq.arith(node.vxyFloor4.length), [function (i) { return mx.affine3_scale([1, 1, (2 + i % 3) / 2]); }, function (_) { return mx.affine3_translate([nr25 * nInner, 0, nFloorD / 2]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor4[i]); }]));
    node.g4RoofC2 = al.duplicate_f(gObj1, al.compose_v4map(seq4Obj1, [function (i) { return mx.affine3_translate([nr25 * nInner, 0, (2 + i % 3) / 2 + nFloorD / 2]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor4[i]); }]));
    node.g4RoofC3 = al.duplicate_f(gObj2, al.compose_v4map(seq4Obj2, [function (i) { return mx.affine3_translate([nr25 * nInner, 0, (2 + i % 3) / 2 + nFloorD / 2]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor4[i]); }]));
    node.g6RoofC1 = al.duplicate_f(gCrys, al.compose_v4map(seq.arith(node.vxyFloor6.length), [function (i) { return mx.affine3_scale([1, 1, (2 + 2 * (i % 2)) / 2]); }, function (_) { return mx.affine3_translate([nr25 * nInner, 0, nFloorD / 2]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor6[i]); }]));
    node.g6RoofC2 = al.duplicate_f(gObj1, al.compose_v4map(seq6Obj1, [function (i) { return mx.affine3_translate([nr25 * nInner, 0, (2 + 2 * (i % 2)) / 2 + nFloorD / 2]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor6[i]); }]));
    node.g6RoofC3 = al.duplicate_f(gObj2, al.compose_v4map(seq6Obj2, [function (i) { return mx.affine3_translate([nr25 * nInner, 0, (2 + 2 * (i % 2)) / 2 + nFloorD / 2]); }, function (i) { return rot_z_atan2_m4(node.vxyFloor6[i]); }]));
    var mapsBottom = al.compose_m4([vc.v2(1, 0), vc.v2(1, -0.25), vc.v2(0.75, -0.5), vc.v2(0.5, -1.5), vc.v2(0.25, -6), vc.v2(1 / 16, -30)].map(function (v) { return v.sub(vc.v2(0, nUnderFloorD)); }), [function (d) { return mx.affine3_scale([d.x, d.x, 1]); }, function (d) { return mx.affine3_translate([0, 0, d.y]); }]);
...
(function (link) {
    ...
    var gStairStep = prim.cube(0.5)
        .map3(function (v) { return mx.affine3_scale([nStairStepX, nStairStepY, nStairDepth]).mul(mx.affine3_translate([0.5, 0, -0.5])).map_v3(v, 1); });
    var gRing = prim.prism(4, nObjR1, nStairDepth * 2)
        .map3(function (v) { return mx.affine3_translate([nObjX, 0, -nStairDepth * 1.5]).map_v3(v, 1); });
    var gCrys = prim.bipyramid(4, nObjR2, ut.phi / (1 + ut.phi), 1 / (1 + ut.phi))
        .map3(function (v) { return mx.affine3_translate([nObjX, 0, 1 / (1 + ut.phi)]).map_v3(v, 1); });
    var gObj1 = exports.g1oICube
        .map3(function (v) { return mx.affine3_translate([nObjX, 0, nObjR1]).mul(mx.affine3_scale([nObjR3, nObjR3, nObjR3])).map_v3(v, 1); });
    var gObj2 = prim.dodecahedron(0.5)
        .map3(function (v) { return mx.affine3_translate([nObjX, 0, nObjR1]).mul(mx.affine3_scale([nObjR3, nObjR3, nObjR3])).map_v3(v, 1); });
    ...
    var gdStairSteps = seq.arith(nStairCount).map(function (i) { return al.concat_surface_models([
        merge_geos('floor', [gStairStep.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, 0]).map_v3(v, 1); })]),
        merge_geos('wall', gRingPair.map(function (g) { return g.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, 0]).map_v3(v, 1); }); })),
        merge_geos('wall', i % 2 != 0 ?
            [] :
            gCrysPair.map(function (g) { return g.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, nObjZ]).mul(mx.affine3_scale([1, 1, seqOverZ[i]])).map_v3(v, 1); }); })),
        merge_geos('roof', i % ((nStairCount - 1) / 2) != 0 ?
            gObj1Pair.map(function (g) { return g.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, nObjZ + seqOverZ[i]]).map_v3(v, 1); }); }) :
            gObj2Pair.map(function (g) { return g.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, nObjZ + seqOverZ[i]]).map_v3(v, 1); }); })),
        merge_geos('wall', i % 2 != 0 ?
            [] :
            gCrysPair.map(function (g) { return g.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, -nObjZ - nStairDepth]).mul(mx.affine3_scale([1, 1, -seqUnderZ[i]])).map_v3(v, 1); }); })),
        merge_geos('wall', i % ((nStairCount - 1) / 2) != 0 ?
            gObj1Pair.map(function (g) { return g.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, -nObjZ - nStairDepth - seqUnderZ[i] - nObjR3 * 1.5]).map_v3(v, 1); }); }) :
            gObj2Pair.map(function (g) { return g.map3(function (v) { return mx.affine3_translate([i * nStairStepX, 0, -nObjZ - nStairDepth - seqUnderZ[i] - nObjR3 * 1.5]).map_v3(v, 1); }); })),
    ]); });
    ...
    function gdLink(c) {
        ...
        var maps = al.compose_v3map(seq.arith(count), [
            function (i) { return mx.affine3_translate([0, 0, seqZ[i]]); },
            function (_) { return mRotZ; },
            function (_) { return mTransV1; },
        ]);
        return al.concat_surface_models(seq.arith(count).map(function (i) { return gdStairSteps[i].map3(function (v) { return maps[i](v); }); }));
    }
    link.gdLink = gdLink;
...
function hexaObj() {
    al.duplicate_f(prim.bipyramid(4, 0.25, 0.5, 0.5), al.compose_v4map(seq.arith(4), [
        function (_) { return mx.affine3_rotate_x(ut.deg90); },
        function (_) { return mx.affine3_translate([3, 0, 0]); },
